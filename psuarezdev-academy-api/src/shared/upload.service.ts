...
export class UploadService {
  ...
  async getFile(dir: UploadPaths, fileName: string) {
    try {
      const path = join(this.uploadsPath, dir, fileName);
  ...
  async uploadFile(dir: UploadPaths, file: File) {
    try {
  ...
  async removeFile(dir: UploadPaths, fileName: string) {
    try {
      const path = join(this.uploadsPath, dir, fileName);
      await unlink(path);
      return true;
